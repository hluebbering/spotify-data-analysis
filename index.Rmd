...
```{r, out.width="100%"}
kable(df.description, escape = F) %>%
  kable_styling(bootstrap_options = c("hover", "striped"), full_width = T, html_font = "Roboto", font_size = 12) %>%
  row_spec(0:22, hline_after = T, extra_css = "text-align: left;") %>%
  column_spec(1, color = "#737373", extra_css = "font-weight: 400; text-transform: uppercase; letter-spacing: 1.25px; font-size: 10px; font-family: roboto;") %>%
  column_spec(2:3, color = "#555", extra_css = "font-weight: 300 !important; font-family: Roboto Condensed;") %>%
  row_spec(0, color = "#111111", extra_css = "text-transform: uppercase; letter-spacing: 1.25px; font-weight: 400; font-size: 11px;") %>%
  kable_paper() %>% 
  scroll_box(height = "2.25in", extra_css = "border: none; padding: 2pt; margin: auto;")
```
...
```{r}
ggplotly(pQ2, orientation = 'h') %>%
  layout(legend = list(
    x = 0.03153023064673607, 
    y = 0.9486015633255951))
...
data.frame(m, s, n) %>% arrange(m) %>% 
  kable(col.names = c("mean ($\\mu_i$)", "std dev ($\\sigma_i$)", "size ($n$)"), escape = F) %>%
  kable_styling(bootstrap_options = c("hover", "striped"), full_width = T, html_font = "Roboto", font_size = 12) %>%
  row_spec(0:2, hline_after = T, extra_css = "text-align: left;") %>%
...
pcor <- ggcorrplot(correlation_matrix, hc.order = TRUE, type = "lower", outline.col = "white", lab = TRUE, lab_size = 2.5, colors = c("#368E25", "#F3FDE7", "#00AFBB")) +
                   # colors = c("#20d761", "#FFFCCC", "#FF47B9")) +
  theme_ipsum_rc(base_family = "Roboto Condensed") + theme(panel.grid = element_blank(), panel.grid.major = element_blank(), plot.margin =  margin(6,6,6,6), axis.text.x = element_text(size = 8, angle = 45), axis.text.y = element_text(size = 8), axis.title.x = element_blank(), axis.title.y = element_blank())

ggplotly(pcor) %>% layout(autosize = T)
...
```{r}
dfQ3$predicted = model4$fitted.values
dfQ3$residual = model4$residuals
...
```{r, out.width="100%"}
dfQ3R1 <- data.frame("Estimate" = c("2.521", "1.003","5.081e-07", "-1.364e-01", "-5.448e-01", "-7.221e-02", "-6.841e-01", "1.117e-03", "1.877e-01"), "P Value" = c("$\\lt$ 2e-16", "$\\lt$ 2e-16", "0.045577", "0.108633", "0.000215", "0.008720", "9.82e-11", "0.011740", "0.005147"), row.names = c("(Intercept)", "danceability", "duration", "energy", "instrumentalness", "mode", "speechiness", "tempo", "valence"))

kable(dfQ3R1) %>%
  kable_styling(bootstrap_options = c("hover", "striped"), full_width = T, html_font = "Roboto", font_size = 12) %>%
  row_spec(0:9, hline_after = T, extra_css = "text-align: left;") %>%
  column_spec(1, color = "#777777", extra_css = "font-weight: 400; text-transform: uppercase; letter-spacing: 1.25px; font-size: 11px;") %>%
  column_spec(2:3, color = "#555", extra_css = "font-weight: 300 !important; font-family: Roboto Condensed;") %>%
  row_spec(0, color = "#111111", extra_css = "text-transform: uppercase; letter-spacing: 1.25px; font-weight: 400; font-size: 11px;") %>%
  kable_paper()
